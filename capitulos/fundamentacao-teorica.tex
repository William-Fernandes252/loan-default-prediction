...
A \textbf{acurácia} de um modelo é definida como a taxa de acertos das previsões:
\begin{center}
...
A \textbf{precisão} mede a proporção de acertos entre as previsões positivas:
\begin{center}
  Precisão = $\frac{TP}{TP + FP}$
  \label{eq:precisao}
\end{center}

A \textbf{especificidade} é oposto da sensibilidade, medindo a capacidade do modelo de prever corretamente instâncias negativas:
\begin{center}
  Especificidade = $\frac{TN}{TN + FP}$
  \label{eq:especificidade}
\end{center}

Analisando estas definições, fica claro porque elas não são suficientes para avaliar a performance de forma assertiva em um cenário de desbalanceamento entre as classes. Se um modelo simplesmente classificasse todas as instâncias de um conjunto como negativas, onde essa classe representasse, por exemplo, \(99\%\), a acurácia seria \(99\%\). Apesar de parecer promissor, tal modelo seria inútil na prática, pois não identifica nenhum caso de interesse~\cite{Naidu2023}. A sua sensibilidade nula e precisão indefinida (por conta de uma divisão por \(0\)) torna isso evidente. Métricas compostas, como \textit{F1-score}, acurácia balanceada, \textit{G-mean}, combinam as métricas básicas para possibilitar uma análise mais equilibrada e assertiva nesse contexto.

O \textbf{\textit{F1-score}} foi criado para balancear a sensibilidade e a precisão de um modelo, sendo definido como a média harmônica entre estes dois indicadores~\cite{Grandini2020}:
\begin{center}
  \textit{F1-score} = $\frac{Precisão \cdot Sensibilidade}{Precisão + Sensibilidade}$
  \label{eq:f1-score}
\end{center}

A \textbf{acurácia balanceada} é uma das correções mais diretas e robustas para o problema da acurácia padrão. Em vez de considerar a média de todas as previsões, a acurácia balanceada calcula o desempenho médio de cada classe individualmente e depois faz uma média. Isso garante que um modelo bem avaliado por essa métrica é eficaz em todas as classes~\cite{Grandini2020}. No caso binário, ela é definida como a média entre a sensibilidade e a especificidade do modelo, que representam, respectivamente, os acertos nas classes positiva e negativa:
\begin{center}
...
Por fim, a área sob a curva \textit{Receiver Operating Characteristic} (ROC), ou \textbf{AUC}, revela a capacidade do modelo em separar as instâncias das classes positiva e negativa, através de uma análise da relação entre a Taxa de Verdadeiros Positivos (TPR) e a Taxa de Falsos Positivos (FPR) para diferentes limiares de decisão~\cite{Naidu2023}. Essa métrica pode ser interpretada da seguinte maneira:
\begin{itemize}
  \item $\textbf{AUC = 1}$: indica que todas as amostras positivas tem maior probabilidade de serem classificadas corretamente, ou seja, um modelo ideal;
  \item $\textbf{AUC = 0.5}$: um modelo com desempenho igual a uma previsão aleatória. A curva ROC segue a linha diagonal do gráfico;
  \item $\textbf{0.5 < AUC <= 1}$: um modelo com poder de discriminação.
\end{itemize}

Por depender apenas da sensibilidade e da especificidade do modelo, a AUC não é afetada pela distribuição das classes. Naidu, Zuva e Sibanda (\citeyear{Naidu2023}) mostraram que, embora métricas como acurácia, precisão e \textit{recall} e \textit{F1-score} sejam mais utilizadas na prática, a AUC é mais perspicaz quanto a adequação da eficácia de um algoritmo.
