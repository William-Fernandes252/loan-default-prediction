...
Inicialmente, considerou-se a utilização do algoritmo AdaBoost como representante da família de \textit{boosting}. No entanto, optou-se pela utilização do XGBoost devido à sua maior robustez e capacidade de generalização. Proposto por Chen e Guestrin (\citeyear{Chen2016}), o ele é uma implementação escalável de \textit{tree boosting} que tem alcançado ótimos resultados em diversos desafios de aprendizado de máquina e aplicações industriais. O XGBoost supera o \textit{boosting} tradicional (como o AdaBoost) por incorporar regularização (\(L_1\) e \(L_2\)) na sua função objetivo, o que ajuda a prevenir o \textit{over-fitting}. Além disso, ele possui um mecanismo nativo para lidar com esparsidade e valores ausentes, aprendendo direções padrão para dados incompletos, e oferece maior estabilidade numérica, evitando erros de execução como divisões por zero durante o treinamento.
