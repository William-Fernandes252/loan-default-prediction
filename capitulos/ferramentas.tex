...
\begin{table}[h]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{|l|l|l|l|}
      ...
      \textbf{Proporção Minoritária} & \(\sim 1,1\%\) (Inadimplentes) & \(22\%\) (Inadimplentes) & \(2,8\%\) (\texttt{D, C ou CC}) \\ \hline
    \end{tabular}%
  }
\end{table}
...
Para mitigar a influência da aleatoriedade na partição dos dados sobre os resultados, cada experimento foi executado \(5\) vezes com sementes distintas, e os resultados finais foram calculados como a média dessas execuções. Em cada execução, os dados foram segmentados em conjuntos de treino (\(70\%\)) e teste (\(30\%\)) por meio de amostragem estratificada. Adicionalmente, empregou-se o método de validação cruzada \textit{k-fold} no conjunto de treino para a otimização de hiper-parâmetros. Os algoritmos de classificação selecionados foram \textit{Support Vector Machine} (SVM), \textit{Random Forest}, XGBoost e \textit{Multi-layer Perceptron} (MLP), utilizando suas implementações da biblioteca \textit{Scikit-learn}~\cite{Pedregosa2011scikit}.
...
\end{itemize}
Para representar as técnicas de aprendizado sensível a custos, foi utilizado o \textbf{\textit{Cost-sensitive SVM} (CSSVM)}, uma implementação do SVM sensível a custos proposta por Iranmehr, Masnadi-Shirazi e Vasconcelos (\citeyear{Iranmehr2019}).

Dada a ausência de uma matriz de custos predefinida para os conjuntos de dados, os custos de classificação incorreta foram tratados como hiper-parâmetros. Estes foram otimizados por meio de busca em grade (\textit{grid search}), uma metodologia também adotada por Cao, Zhao e Zaiane (\citeyear{Cao2013}), utilizando a grade da Tabela \ref{tab:grid-search-params}. Por convenção, este trabalho rotula a classe minoritária (ex: inadimplentes) como positiva (1) e a classe majoritária (ex: adimplentes) como negativa (0).

\begin{table}[H]
  \centering
  \caption{Grade de hiperparâmetros utilizada na busca em grade (\textit{Grid Search})}
  \label{tab:grid-search-params}
  \begin{tabular}{|l|l|l|}
    \hline
    \textbf{Algoritmo} & \textbf{Hiperparâmetro} & \textbf{Valores Testados} \\ \hline
    \multirow{4}{*}{MLP} & Camadas Ocultas & (50,), (100,), (50, 50) \\
    & Função de Ativação & relu, tanh \\
    & Alpha (Regularização) & 0.0001, 0.01 \\
    & Early Stopping & True \\ \hline
    \multirow{3}{*}{Random Forest} & Nº de Estimadores & 100, 200 \\
    & Profundidade Máxima & 5, 10, 20 \\
    & Min. Amostras por Folha & 1, 5 \\ \hline
    \multirow{5}{*}{SVM} & Loss & log\_loss \\
    & Alpha (Regularização) & 0.0001, 0.001, 0.01 \\
    & Penalty & l2, elasticnet \\
    & Max Iter & 1000, 2000 \\
    & Tolerância & 0.001 \\ \hline
    \multirow{7}{*}{XGBoost} & Nº de Estimadores & 100, 200 \\
    & Taxa de Aprendizado & 0.01, 0.1 \\
    & Profundidade Máxima & 3, 6, 10 \\
    & Subamostra (Linhas) & 0.8, 1.0 \\
    & Subamostra (Colunas) & 0.8, 1.0 \\
    & Regularização L1 (\textit{Alpha}) & 0, 0.1 \\
    & Regularização L2 (\textit{Lambda}) & 1.0, 10.0 \\ \hline
    \multirow{1}{*}{Custos} & Pesos da Classe Minoritária (fator de custo) & 1, 5, 10, balanced \\ \hline
  \end{tabular}
\end{table}

\section{Métricas de Avaliação}\label{sec:metricas}

A avaliação do desempenho dos modelos baseou-se em métricas robustas ao desbalanceamento de classes, especificamente: acurácia balanceada, G-mean, sensibilidade, especificidade, precisão e F1-score~\cite{Namvar2018}. A sensibilidade e a precisão são particularmente relevantes para o contexto de risco de crédito, pois refletem, respectivamente, a capacidade do modelo de identificar corretamente os inadimplentes e a proporção de previsões positivas que são realmente corretas~\cite{Wei2025}. Dado que o equilíbrio entre ambas é frequentemente afetado por estratégias de tratamento de dados, onde o aumento da sensibilidade pode levar a uma redução na precisão, utilizou-se o F1-score para verificar o equilíbrio entre essas métricas.

\section{Código-fonte dos Experimentos}

O código-fonte utilizado para a execução dos experimentos está disponível no repositório do projeto, acessível em~\url{https://github.com/William-Fernandes252/loan-default-prediction}.