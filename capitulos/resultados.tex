...
Nesta seção, serão discutidos os resultados parciais dos experimentos, onde foi obtida a média das métricas obtidas no treinamento para 5 seeds de cada combinação de modelo (MLP, \textit{Random Forest}, SVM e XGBoost) e técnica (CSSVM, Linha de base, RUS, MetaCost, SMOTE e SMOTE Tomek), resultando em \(120\) execuções com o conjunto de dados de cartão de crédito de Taiwan.
...
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_cssvm.tex}}
  \caption{Resultados do CSSVM (média de 5 sementes)}
  \label{tab:resultados-cssvm}
\end{table}
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_linha_de_base.tex}}
  \caption{Resultados com treinamento direto}
  \label{tab:resultados-linha-base}
\end{table}
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_meta_cost.tex}}
  \caption{Resultados do MetaCost (média de 5 sementes)}
  \label{tab:resultados-meta-cost}
\end{table}
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_rus.tex}}
  \caption{Resultados do RUS (média de 5 sementes)}
  \label{tab:resultados-rus}
\end{table}
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_smote.tex}}
  \caption{Resultados do SMOTE (média de 5 sementes)}
  \label{tab:resultados-smote}
\end{table}
\begin{table}[H]
  \centering
  {\fontsize{8}{11}\selectfont \input{experimentos/reports/pt_BR/taiwan_credit/figures/results_summary_smote_tomek.tex}}
