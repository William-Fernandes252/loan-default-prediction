...
from experiments.core.data.protocols import ProcessedDataExporter
...
class DescribeParquetDataExporter:
    ...
    def it_satisfies_processed_data_exporter_protocol(
        self, tmp_path: Path, storage: StorageService
    ) -> None:
        """Verify ParquetDataExporter satisfies the protocol."""
        exporter = ParquetDataExporter(storage, str(tmp_path))
        assert isinstance(exporter, ProcessedDataExporter)

    def it_exports_dataframe_to_parquet(
        self, tmp_path: Path, sample_dataframe: pl.DataFrame, storage: StorageService
    ) -> None:
        """Verify exporter writes DataFrame to Parquet file."""
        exporter = ParquetDataExporter(storage, str(tmp_path))

        output_uri = exporter.export(sample_dataframe, Dataset.TAIWAN_CREDIT)

        output_path = Path(output_uri)
        assert output_path.exists()
        assert output_path.suffix == ".parquet"

    def it_returns_correct_output_uri(
        self, tmp_path: Path, sample_dataframe: pl.DataFrame, storage: StorageService
    ) -> None:
        """Verify returned URI matches expected location."""
        exporter = ParquetDataExporter(storage, str(tmp_path))

        output_uri = exporter.export(sample_dataframe, Dataset.TAIWAN_CREDIT)

        expected_path = tmp_path / "taiwan_credit.parquet"
        assert output_uri == str(expected_path)
    ...
    def it_creates_parent_directories(
        ...
        exporter = ParquetDataExporter(storage, str(nested_path))

        output_uri = exporter.export(sample_dataframe, Dataset.LENDING_CLUB)
    ...
    def it_preserves_dataframe_content(
        ...
        loaded_df = pl.read_parquet(output_uri)
    ...
    def it_overwrites_existing_file(
        ...
        output_uri_2 = exporter.export(new_df, Dataset.TAIWAN_CREDIT)

        assert output_uri == output_uri_2
