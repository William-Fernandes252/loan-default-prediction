...
class DescribeParquetResultsLoader:
    ...
    def it_adds_model_display_column(self, results_path: Path):
        loader = ParquetResultsLoader(
            path_provider=FakePathProvider(results_path),
            translate=identity_translate,
        )
        ...
        # The display name should be translated model name
        assert result["model_display"].iloc[0] is not None

    def it_adds_technique_display_column(self, results_path: Path):
        loader = ParquetResultsLoader(
            path_provider=FakePathProvider(results_path),
            translate=identity_translate,
        )
    ...
    def it_handles_dataframe_without_model_column(self, tmp_path: Path):
        df = pd.DataFrame({"accuracy_balanced": [0.85, 0.82]})
        path = tmp_path / "no_model.parquet"
        df.to_parquet(path)

        loader = ParquetResultsLoader(
            path_provider=FakePathProvider(path),
            translate=identity_translate,
        )
    ...
    def it_handles_dataframe_without_technique_column(self, tmp_path: Path):
        df = pd.DataFrame({"model": ["rf"], "accuracy_balanced": [0.85]})
        path = tmp_path / "no_technique.parquet"
        df.to_parquet(path)
